...
angular.module('myApp',[])
    ...
    .controller('ShoppingListController', function($scope, $http, $log, helperFactory,MAX_LENGTH,MIN_LENGTH){
        ...
        $scope.howManyMoreCharactersNeeded = function(){
            var charecters = (MIN_LENGTH - $scope.item.length);
            return (charecters > 0) ? charecters : 0;
        };
        $scope.howManyCharactersRemaining = function(){
            var charecters = (MAX_LENGTH - $scope.item.length);
            return (charecters > 0) ? charecters : 0;
        };
        ...
        $scope.anyCharactorsOver = function(){
            return ($scope.howManyCharactersOver() > 0);
        };
        $scope.isNumberOfCharactersWithinRange = function(){
            return(
               $scope.minimumCharactersMet()  &&
                   !$scope.howManyCharactersOver()
            );
        };
        $scope.goodToGo = function(){
            return(

                //this is for the add button... if one of these is missing then button wont be set to true
        ...
        $scope.select = function(){
            //this time we using the short version because we are just calling php file. not sending anything
            $http.get(urlSelect)
                .success(function(data){
                    if(data.items){
